...
- Выполняйте `dotnet workload restore` и target-specific build только для target framework из утверждённого стека; передавайте target framework команде `dotnet build` через ключ `-f`.
- Единое соглашение для placeholders: маркер в двух фигурных скобках с именем в верхнем регистре заменяется только из утверждённого `technology-stack.md` и проверяемых файлов проекта. Имя `DOTNET_MAUI_TARGET` означает утверждённый target framework для ключа `-f`. Неразрешённый placeholder означает ошибку; такую команду нельзя выполнять.
